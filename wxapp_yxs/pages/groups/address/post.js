...
Page({
  ...
  submit: function () {
    ...
    if (!t.data.posting) {
      ...
      if (!i.datavalue && !i.areas) return void a.toast(t, "地址数据出错，请重新选择");
      
      i.id = t.data.id, t.setData({
        posting: !0
      }), e.post("groups/address/submit", i, function (i) {
        // a.toast(t, i.message);return false;
        if (1 != i.status) return t.setData({
          posting: !1
        ...
        t.setData({
          ...
          "member" == t.data.type ? wx.navigateBack() : wx.redirectTo({
            url: "/pages/groups/address/selector"
          })
        }, 1e3)
      })
    }
